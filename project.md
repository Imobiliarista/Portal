...
**Última atualização:** Corrigido gap estrutural em `corretores/{slug}.json` — a aprovação do corretor nunca enfileirava a geração deste artefato (único gatilho existente era mutação de anúncio), deixando corretor aprovado sem nenhum anúncio permanentemente sem esse JSON; ver Histórico de Decisões
**Versão:** 1.19

...
**Última atualização:** Corrigidos os consumers da fila principal e da dead letter queue (`src/queue.ts`, `src/queue-dlq.ts`, Lote 23) — mensagem malformada (corpo nulo/indefinido ou sem campo `tipo` em string) derrubava o processamento do resto do batch com um `TypeError` não capturado; agora é descartada (ack, sem retry) e logada sem afetar as demais mensagens; ver Histórico de Decisões
**Versão:** 1.20

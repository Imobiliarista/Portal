...
**Última atualização:** Auditoria — corrigidas 4 menções enganosas de "Asaas já implementado em sandbox" (seções 2.1, 2.2, 3 e Histórico de Decisões) para "planejado, nunca implementado"; histórico completo de git (66 branches) confirmou que não existe nenhum código de integração Asaas em lugar nenhum do repositório, só a variável vazia `ASAAS_SANDBOX_API_KEY` em `.env.example` — ver achado em 11
**Versão:** 1.14
...
- **Sistema de Planos** (necessidade técnica de custo **e** modelo comercial — ver 5.1, 6.3, 6.4):
  ...
  * Módulo de integração com **Asaas**: planejado, nunca implementado — variável de ambiente reservada em `.env.example` (`ASAAS_SANDBOX_API_KEY`), sem código correspondente (ver achado de auditoria em 11).
...
- **Cobrança ativa via Asaas** (planejado, nunca implementado — variável de ambiente reservada em `.env.example`, sem código correspondente; ver achado de auditoria em 11).
...
| Cobrança (futuro)          | Asaas — planejado, nunca implementado. Variável de ambiente reservada em `.env.example`, sem código correspondente (ver achado de auditoria em 11)                         |                                                                                                                                 |

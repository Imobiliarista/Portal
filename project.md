...
**Última atualização:** Lote 23 — consumer + alerta por e-mail pra `imob-queue-dlq` (segundo `[[queues.consumers]]` em wrangler.toml, roteado por `batch.queue` dentro do único export `queue()` do Worker); fecha a pendência real registrada no Lote 22 (nenhum consumer vinculado à DLQ)
**Versão:** 1.13

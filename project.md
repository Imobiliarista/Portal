...
| 14 | Sistema de Planos expandido | Migration nova (`00XX_planos.sql`) — tabela `planos` com os 5 níveis de referência (5.1.3); `db/queries-planos.ts`; CRUD no `painel-superadmin.ts` (6.3); regras de troca de plano (6.4); Promoção de Lançamento (6.5); Controle de Isenção genérico (6.6) — campos `isento`/`isentoAte`/`motivoIsencao` no Corretor + Cron de reversão automática | 🔲 Não iniciado |

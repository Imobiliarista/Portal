...
**Última atualização:** Três correções independentes de baixo risco, seguindo diagnóstico de sessão anterior: (1) dashboard do Superadmin envelopado corretamente (`GET /api/painel-admin/visao-geral` passa a responder `{ dados: {...} }`, mesmo padrão do resto de `painel-superadmin.js`); (2) nova rota `POST /api/painel-admin/regenerar-cidade/:id` pra forçar regeneração manual de uma cidade sem depender de editar um anúncio dela; (3) `lib/r2.ts::escreverJSON` corrigido pra gravar Content-Type/Cache-Control em `httpMetadata` (não `customMetadata`); ver Histórico de Decisões
**Versão:** 1.21
...
| **Correção — dashboard do Superadmin lia `dados.dados.X`, backend respondia plano (sem envelope)** | **Fecha o diagnóstico de sessão anterior** (console em produção mostrava `TypeError: Cannot read properties of undefined (reading 'corretores_totais')` em `painel-superadmin.js:79`, todas as métricas do dashboard como "-"). **Causa raiz confirmada lendo o código, não presumida:** `GET /api/painel-admin/visao-geral` (`routes/painel-superadmin.ts::rotaVisaoGeral`) sempre respondeu **200 OK**, nunca 500 — não é exceção de backend, é mismatch de formato. `respostaSucesso(visao)` fazia só `JSON.stringify(visao)`, um objeto plano (`{ corretores_totais, ... }` direto na raiz); `painel-superadmin.js:79-85` lê `dados.dados.corretores_totais`, esperando um envelope `{ dados: {...} }` — `dados.dados` vinha `undefined`, e ler `.corretores_totais` disso é exatamente o `TypeError` reportado. **Não é regressão das PRs #67-75:** nem `painel-superadmin.js` nem a rota `visao-geral` foram tocados nesse intervalo — o mismatch existe desde a criação do endpoint (commit `c1c49f6`, 2026-08-14, antes até da PR #28). **Qual lado corrigir, decidido pelo padrão majoritário do próprio arquivo:** as outras 4 chamadas de `painel-superadmin.js` (`carregarMinisites`, `carregarPreCadastros`, `carregarCidades`, `carregarModulos`) todas leem `dados.dados`, e os handlers correspondentes em `painel-superadmin.ts` todos respondem `respostaSucesso({ dados: ... })` — `rotaVisaoGeral` era a única exceção no backend. Corrigido o lado minoritário: `respostaSucesso({ dados: visao })`, sem tocar no front (que já seguia o padrão certo) nem inventar um terceiro formato. **Teste que reproduz o bug antes da correção, travado em `test/dashboard-visao-geral.test.ts`:** login real de superadmin, `GET /api/painel-admin/visao-geral`, espera `corpo.dados.corretores_totais` numérico; rodado contra o código anterior à correção falhou com `expected undefined to be defined` (a mesma leitura que o dashboard real faz), confirmando o bug reproduzido, não hipotético; depois da correção, passa. Teste de guarda adicional: `401` sem cookie de sessão. Validado: `npm run typecheck` (0 erros), `npm run lint` (exit 0), `npm test` (31/31, nenhuma quebra nos 24 já existentes) | Nem todo bug de "campo undefined" é uma exceção de backend — a rota respondia 200 com JSON válido o tempo todo; o formato certo pra corrigir (front vs. back) se decide pelo padrão já estabelecido no resto do arquivo, não por preferência arbitrária, e olhar só a rota isolada (sem comparar com as irmãs no mesmo arquivo) teria escondido que o backend, não o front, era o outlier |
| **Nova rota — regeneração manual de cidade pelo Superadmin (`POST /api/painel-admin/regenerar-cidade/:id`)** | **Fecha o segundo achado do diagnóstico de sessão anterior**, sobre anúncios de teste inseridos direto no D1 via SQL que não apareciam na busca de Londrina mesmo depois de editar e salvar um anúncio existente do corretor pelo painel. **Não é bug em `jobs/revalidacao-cruzada.ts` — comportamento correto, deliberadamente não alterado:** `enfileirarRevalidacaoDoAnuncio`/`processarRevalidacaoCruzada` (fan-out por mutação de anúncio) sempre revalida a **cidade do anúncio editado** (`anuncio.cidade_id`, lido em `api-anuncios-crud.ts:395/403/413`), nunca uma cidade fixa — é assim que deve funcionar pra uma edição normal. O gap real: **não existia nenhuma forma de forçar a regeneração de uma cidade arbitrária** sem editar (de verdade) um anúncio pertencente a ela — o cenário do diagnóstico anterior editou um anúncio de outra cidade, então `cidades/londrina.json` nunca foi tocado, apesar dos 10 anúncios de teste estarem corretos no D1 (`postar_na_rede=1`, `vendido_removido=0`, `cidade_id` de Londrina). **Correção, reaproveitando a função que o fan-out normal já usa, sem duplicar lógica:** nova `enfileirarGeracaoJsonCidade(env, cidade_id, cidade_slug)` em `jobs/gerar-json-cidade.ts` — mesmo padrão já existente de `jobs/gerar-json-corretor.ts::enfileirarGeracaoJsonCorretor` (usada, por sinal, pelo mesmo tipo de gatilho administrativo: aprovação de corretor pelo Superadmin, já em `painel-superadmin.ts`) — envia exatamente o mesmo tipo de mensagem (`{ tipo: "gerar-json-cidade", cidade_id, cidade_slug }`) que `processarRevalidacaoCruzada` já envia no fan-out normal, só que disparada direto, sem depender de mutação de anúncio. `slugificarCidade` (antes privada em `revalidacao-cruzada.ts`) exportada e reaproveitada pela nova rota, em vez de duplicar a regra de slug uma terceira vez (já existia uma cópia equivalente em `gerar-json-cidade.ts::slugificar`, usada pra outro fim — partição por bairro). Nova rota `rotaRegenerarCidade` protegida por `obterSuperadminIdDaSessao`, o mesmo middleware de todas as outras rotas administrativas deste arquivo; busca a cidade via `buscarCidade` (`db/queries-superadmin.ts`, já existente) e responde `404` se o `cidade_id` não existir. **Teste que reproduz o gap antes da correção, travado em `test/regenerar-cidade.test.ts`:** com a rota desativada, os três cenários (`401` sem sessão, `404` pra cidade inexistente, `200` + mensagem enfileirada) falham todos com `404 Rota não encontrada` — confirmando que não havia caminho nenhum pra esse disparo manual; depois de reativada, passam. Teste de ponta a ponta reproduz o cenário exato do diagnóstico: insere um anúncio direto via SQL (`INSERT INTO anuncios` sem passar pela API — nenhuma revalidação disparada por essa inserção), confirma que `cidades/{slug}.json` não existe em R2 antes, chama a nova rota, materializa `processarGerarJsonCidade` de verdade (bypassando o transporte da Queue, mesma abordagem de `funil-completo.test.ts`/`sitemap-job-trigger.test.ts`) e confirma que o anúncio aparece no JSON regenerado. Validado: `npm run typecheck` (0 erros), `npm run lint` (exit 0), `npm test` (31/31, nenhuma quebra nos 24 já existentes) | Um fan-out "correto" (revalida a cidade certa pra uma mutação normal) ainda pode deixar um gap operacional — dado inserido fora do fluxo normal (seed manual, correção direta no banco) não tem mutação nenhuma pra disparar o fan-out, e forçar essa regeneração via "edite qualquer anúncio do corretor" só funciona por acidente, se esse anúncio por acaso pertencer à cidade certa; a correção certa aqui não era mudar o comportamento existente (que está certo), era adicionar um gatilho manual explícito, reaproveitando a mesma função de enfileiramento já usada pelo caminho automático |
| **Correção — `lib/r2.ts::escreverJSON` gravava Content-Type/Cache-Control em `customMetadata`, nunca viravam headers HTTP reais** | **Achado ao investigar a hipótese de CDN servindo versão antiga do JSON (terceiro ponto do diagnóstico de sessão anterior sobre anúncios de teste não aparecerem na busca) — não era a causa da staleness (ver linha acima, causa raiz era outra), mas é um bug real, de baixo risco corrigir separadamente.** `escreverJSON` (usado por `gerar-json-cidade.ts` e `gerar-json-corretor.ts`, os dois jobs que materializam os JSONs públicos em R2) gravava `content-type`/`cache-control` dentro de `customMetadata` — campo de metadados arbitrários do R2, que `R2Object::writeHttpMetadata()` **não** lê. `servirR2Publico` (`src/index.ts`, bypass de `dados.imobiliarista.net`/`midias.imobiliarista.net`) usa exatamente esse método pra montar os headers da resposta — então todo JSON de cidade/corretor sempre foi servido **sem** `Content-Type` nem `Cache-Control` reais (headers ausentes, `null`, não um valor errado), apesar do código "parecer" configurar os dois. Outros três pontos do código sempre usaram o campo certo (`httpMetadata: { contentType, cacheControl }`): `scheduled.ts`, `modulos/pwa/logica.ts`, `modulos/feed-portais-independentes/gerador.ts` — só `escreverJSON` estava com o campo trocado. **Efeito prático da ausência de `Cache-Control`:** tende a reduzir cache agressivo na borda (sem diretiva explícita pra o CDN reter), não a causar dado desatualizado — por isso esse bug não explica o sintoma investigado, mas vale corrigir mesmo assim. **Correção:** trocado pra `httpMetadata: { contentType: "application/json", cacheControl: "public, max-age=3600" }`, mesmo padrão já usado corretamente nos 3 arquivos citados acima — não inventado formato novo. **Teste que reproduz o bug antes da correção, travado em `test/r2-metadados-json.test.ts`:** grava um JSON via `escreverJSON`, busca via `dados.imobiliarista.net` (mesmo bypass que o front-end usa de verdade); rodado contra o código anterior à correção confirmou `content-type` **`null`** (não um valor incorreto — ausente por completo), consistente com a causa raiz; depois da correção, `application/json`/`public, max-age=3600` corretos. Validado: `npm run typecheck` (0 erros), `npm run lint` (exit 0), `npm test` (31/31, nenhuma quebra nos 24 já existentes) | `customMetadata` e `httpMetadata` do R2 têm nomes parecidos mas comportamento bem diferente — um vira headers HTTP reais na resposta (`writeHttpMetadata()`), o outro é só um dicionário arbitrário que a aplicação precisaria ler e aplicar manualmente; o bug nunca gerava erro nem log (o `put()` sempre "funcionava"), só um header ausente — mesma classe de defeito silencioso já registrada várias vezes nesta tabela, aqui numa API da plataforma (R2) em vez de lógica de aplicação |
| **Correção — `app-dados.js::fetchCityListings` montava a URL errada pro caso sem partição (`cidades/{cidade}/{cidade}.json` em vez de `cidades/{cidade}.json`), 404 fantasma mesmo com o JSON certo em R2** | **Sintoma:** `https://dados.imobiliarista.net/cidades/londrina.json` respondia `200` com os 10 anúncios de teste corretos (regenerados pela rota da linha "Nova rota — regeneração manual de cidade" acima), mas `https://imobiliarista.net/londrina` mostrava "0 imóvel(is) encontrado(s)", com 404 no Console em `...ina/londrina.json`. **Causa raiz confirmada lendo os dois lados, caractere por caractere:** `fetchCityListings` (`app-dados.js`) buscava primeiro o índice (`cidades/{slug}/_index.json`, formato certo) mas depois lia um campo `index.files` que **nunca existiu** no schema real (`IndiceCidade`, `jobs/gerar-json-cidade.ts`: `total_anuncios`, `particoes?.arquivos`, `modulosAtivos` — sem `files`); no fallback (`index.files || [\`${citySlug}.json\`]`), montava a URL final como `${r2DadosUrl}/cidades/${citySlug}/${file}` — prefixando o slug da cidade de novo em cima de um nome de arquivo que já era só `{slug}.json`. `gerar-json-cidade.ts`, pro caso sem partição (o caminho comum, `items` abaixo do limite de 1MB), grava o arquivo **plano** em `cidades/{slug}.json` — exatamente como a seção 4.4.2 documenta (`"/cidades/londrina.json` (sem partição)"`) — nunca dentro de uma subpasta `cidades/{slug}/`. Resultado: pra qualquer cidade sem partição, o front-end pedia `cidades/londrina/londrina.json` (bate exatamente com o 404 truncado reportado, `"...ina/londrina.json"`), que nunca existiu → `response.ok` falso → lista vazia, sem exceção nem log. **Lado errado: o leitor** (`app-dados.js`), não o escritor — `gerar-json-cidade.ts` já seguia o formato documentado em 4.4.2 dos três ramos (vazio, plano, particionado); o campo assumido pelo front-end (`index.files`) nunca existiu em nenhum dos três. Bug latente também no ramo particionado (não reproduzido em produção ainda, mas mesmo defeito): o índice usa `particoes.arquivos`, um array de **caminhos já completos** (ex.: `cidades/{slug}/{bairro}.json`); `fetchCityListings` teria prefixado `cidades/{slug}/` de novo em cima disso, dobrando o slug. **Correção, só no leitor:** `fetchCityListings` agora usa `index.total_anuncios` pra decidir se há dado (ramo vazio, sem fetch adicional) e `index.particoes ? index.particoes.arquivos : [\`cidades/${citySlug}.json\`]` pra montar a lista de arquivos — sem partição, o único item é o caminho plano documentado; com partição, os caminhos de `particoes.arquivos` são usados como estão, sem prefixo adicional. **Teste que reproduz o bug antes da correção, travado em `test/frontend-fetch-city-listings.test.ts`:** insere 10 anúncios de teste em Londrina (mesmo padrão de `scripts/seed-anuncios-teste.js`), regenera `cidades/londrina.json` chamando `processarGerarJsonCidade` de verdade (mesma lógica de `POST /api/painel-admin/regenerar-cidade/:id`), depois carrega o `fetchCityListings` **real** de `public/assets/js/app-dados.js` (importado via `?raw` e avaliado com `new Function`, não uma reimplementação) e executa contra o mesmo bypass `dados.imobiliarista.net` (`exports.default.fetch`) que o navegador usa de verdade; rodado contra o código anterior à correção, confirmou `appState.allListings` vazio (o mesmo 404 do sintoma relatado, não hipotético); depois da correção, os 10 títulos batem. Validado: `npm run typecheck` (0 erros), `npm run lint` (exit 0), `npm test` (32/32, 1 novo em `test/frontend-fetch-city-listings.test.ts`, nenhuma quebra nos 31 já existentes) | Um endpoint respondendo `200` com o dado certo (`dados.imobiliarista.net/cidades/londrina.json`) não garante que o consumidor real bata a mesma URL — o índice existia precisamente pra evitar essa suposição ("o front-end nunca precisa adivinhar a estrutura", 4.4.2), mas o próprio leitor do índice adivinhava um campo (`index.files`) que o escritor nunca produziu; comparar o contrato dos dois lados (schema gravado vs. schema lido) caractere por caractere, em vez de só confirmar que o R2 tem o dado certo, é o que expôs a causa raiz aqui |

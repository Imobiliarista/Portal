...
**Última atualização:** Corrigido bug de comparação de string na expiração de sessão (`expira_em > datetime('now')` em SQL comparava ISO 8601 contra o formato do SQLite, dando até ~24h de graça além do TTL) — 4 pontos corrigidos pra comparação numérica via `Date`; ver Histórico de Decisões
**Versão:** 1.19

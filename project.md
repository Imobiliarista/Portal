...
**Última atualização:** Lote 22 — dívida de typecheck zerada (51 → 0 erros, categorizados e corrigidos sem mudar comportamento, exceto um caso decidido pelo dono do projeto em `scheduled.ts`); `ci.yml` sem baseline, typecheck agora bloqueante de verdade; nota desatualizada sobre criação da fila `imob-queue-dlq` corrigida (fila já existe — pendência real registrada é a ausência de consumer)
**Versão:** 1.12

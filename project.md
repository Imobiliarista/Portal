...
**Última atualização:** Correção de CORS em `dados.imobiliarista.net`/`midias.imobiliarista.net` — Workers Routes sempre tem precedência sobre R2 Custom Domain no mesmo hostname (comportamento padrão da Cloudflare), então o Worker passou a servir esses dois hostnames fixos direto do bucket R2 correspondente, com CORS restrito a `*.imobiliarista.net`; ver Histórico de Decisões
**Versão:** 1.18

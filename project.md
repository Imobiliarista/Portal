...
**Última atualização:** Módulo Asaas — infraestrutura implementada (`src/services/asaas.ts`, `src/routes/webhooks/asaas.ts`) e desativada por padrão via `ASAAS_ATIVO=false`; fecha o achado de auditoria de 2026-08-19 (Asaas nunca tinha sido implementado, apesar do que o documento afirmava) sem tocar no fluxo real de troca de plano — ver seção 2.3 e Histórico de Decisões
**Versão:** 1.15

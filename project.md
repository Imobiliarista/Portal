...
**Última atualização:** implementação do Lote 17 (Backup/Exportação de Anúncios pelo Corretor) — `GET /api/anuncios/backup`, `POST /api/anuncios/restaurar` (modo seguro, validado contra os limites do Plano) e `GET /api/anuncios/exportar/{portal}` (reaproveita `lib/vrsync-mapper.ts` do Lote 12), em `routes/api-anuncios-backup.ts`; corrigida lacuna de roteamento pré-existente do Lote 5 (`rotasAnuncios` nunca estava montado em `src/index.ts`)
**Versão:** 1.8
...
| 17 | Backup/Exportação de Anúncios pelo Corretor | `routes/api-anuncios-backup.ts` (extraído de `api-anuncios-crud.ts`, já no limite de linhas): `GET /api/anuncios/backup` (schema próprio, fotos só como links R2, download via `Content-Disposition: attachment`), `POST /api/anuncios/restaurar` (modo seguro — só cria por ID inexistente, nunca sobrescreve, valida `maxAnuncios`/`maxFotosPorAnuncio` do Plano antes de gravar, preserva o ID original), `GET /api/anuncios/exportar/{portal}` (mão única, reaproveita `lib/vrsync-mapper.ts` do Lote 12 — ver nota de implementação em 5.1.4-like abaixo). Corrigida lacuna pré-existente: `rotasAnuncios` nunca estava montado em `src/index.ts` (Lote 5 ficava inacessível via HTTP) | 🟢 Concluído |
...
| **Lote 17** | Encontrada e corrigida uma lacuna de roteamento pré-existente do Lote 5: `rotasAnuncios` (`routes/api-anuncios.ts`) nunca era importado nem chamado em `src/index.ts` — todo `/api/anuncios/*` (CRUD completo de anúncios) estava inacessível via HTTP desde que o Lote 5 foi mesclado, sem que nenhum lote posterior notasse. Corrigido com uma delegação de `/api/anuncios*` no `fetch()` principal, no mesmo commit do Lote 17 (sem essa correção os novos endpoints de backup/restauração/exportação também ficariam mortos) | Confirmado por `git log`/leitura direta do `fetch()` antes de codar, não por suposição; consultado o usuário antes de decidir corrigir um arquivo fora do escopo direto do lote (`src/index.ts`), conforme instrução de parar diante de ambiguidade de roteamento |
| **Lote 17** | `GET /api/anuncios/exportar/{portal}` implementado usando o **slug do portal** como identificador da rota (`grupo-olx`, ou qualquer slug de `portais_independentes` como `chaves-na-mao`), não um sufixo `-json`/`-xml` livre como nos exemplos ilustrativos da seção 4.20b. Reaproveita só o que o Lote 12 já construiu (`lib/vrsync-mapper.ts` + geração de XML VRSync) — nenhum mapeador proprietário novo (JSON da OLX, tags próprias do Chaves na Mão) foi criado, porque a própria seção 4.11 já registra esses como mapeadores "futuramente" | A seção 4.20b pede explicitamente para reaproveitar "os mapeadores de taxonomia já existentes do Lote 12", não para construir os mapeadores "futuramente" citados em 4.11; inventar um schema proprietário novo sem especificação real seria escopo não pedido e risco de retrabalho quando o mapeador de verdade for implementado |

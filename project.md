...
**Autoatendimento:** o corretor troca de plano sozinho, pelo próprio
painel — não depende do Superadmin (diferente do fluxo de teste usado
em sessões anteriores, que foi só um atalho administrativo pra validar
a lógica).

**Ciclo de cobrança no upgrade:** a data do upgrade vira a nova 'data
de aniversário' de cobrança mensal do corretor (reinicia o ciclo, não
mantém a data original).

**Cobrança proporcional no mês do upgrade:** a diferença de valor entre
o plano antigo e o novo é cobrada de forma proporcional aos dias já
transcorridos/restantes do ciclo até a data do upgrade — cobrada no
momento do upgrade, não adiada pro próximo ciclo. (Cálculo efetivo só
entra em vigor quando o Asaas for ativado pra cobrança real, fase 3 —
por enquanto, sandbox, sem cobrança disparada de fato, mas a regra já
fica documentada pra implementação futura não precisar redecidir isso.)
...
| **Auditoria de fluxo completo (teste ponta a ponta)** | **Verificado, não corrigido:** a URL de validação do Turnstile em `validarTurnstile` (`src/routes/api-auth-cadastro.ts`) é `https://challenges.cloudflare.com/turnstile/validate` — confirmado via busca na documentação oficial (repositório `cloudflare/cloudflare-docs` no GitHub + página `developers.cloudflare.com/turnstile/get-started/server-side-validation`) que o endpoint correto é `https://challenges.cloudflare.com/turnstile/v0/siteverify`. Isso significa que a validação de Turnstile provavelmente falha sempre em produção também (não só no ambiente de teste, que não tinha saída de rede pra `challenges.cloudflare.com` e não pôde confirmar isso batendo na API real) — todo `POST /api/auth/pre-cadastro` legítimo seria rejeitado com "Validação Turnstile falhou" assim que `TURNSTILE_SECRET_KEY` fosse configurado em produção. Não corrigido nesta sessão a pedido explícito (item fora do escopo dos 6 bugs corrigidos) — fica registrado como próximo item de correção | Documentar o achado pra não se perder antes da próxima sessão de correção; a fonte (docs oficiais da Cloudflare, não memória do modelo) está registrada pra auditoria |
...
| **Auditoria de ações de UI (painel.js/painel-superadmin.js)** | Confirmado o mesmo padrão de "caminho nunca exercitado" nos botões do painel: (1) todo o CRUD de anúncio do corretor (`painel.js`) só disparava `alert()` de placeholder — nunca chamava `/api/anuncios`, apesar da API real (Lote 5) já existir e funcionar; reconectados criar/editar/deletar às rotas reais (`POST`/`PUT`/`DELETE /api/anuncios[/:id]`), com estado de edição (`anuncioEditandoId`) pra alternar entre criar e editar no mesmo formulário. Testado ponta a ponta via `wrangler dev` local: criar, editar e deletar um anúncio real refletiram corretamente na listagem. (2) Os selects estáticos de Tipo de Negócio/Categoria/Tipo de Imóvel/Cidade enviavam slugs de texto (`"venda"`, `"apartamento"`, `"londrina"`) em campos que a API espera como IDs numéricos de `tipos_negocio`/`categorias_imovel`/`tipos_imovel`/`cidades` — corrigidos os `value` das opções existentes pros IDs reais do seed (sem introduzir carregamento dinâmico via API, que fica pra um comando dedicado de taxonomia; a opção "Comercial" de Tipo de Imóvel, que nunca correspondeu a nenhuma linha real da tabela, foi mapeada pro tipo "Loja" da categoria Comercial como escolha provisória, sinalizada pra revisão no comando de taxonomia). (3) Autoatendimento de troca de plano adicionado ao painel do corretor (nova view "Meu Plano"), reaproveitando a mesma regra de negócio de `trocarPlanoDoCorretor` (6.4) via novo endpoint `POST /api/painel-corretor/plano/trocar` — antes só existia o equivalente administrativo (`POST /api/painel-admin/corretor/:id/trocar-plano`). (4) Logout do Superadmin (`painel-superadmin.js`) só redirecionava sem revogar a sessão no servidor (comentário próprio confirmava: "Implementar logout real quando tiver rota de logout") — a rota (`POST /api/auth/logout`) já existia e já era genérica o bastante pra qualquer papel (corretor ou superadmin, mesma tabela `sessoes`); corrigido pra chamar essa rota antes de redirecionar. **Achado não corrigido nesta sessão** (fora do escopo pedido): o logout do corretor (`painel.js::logout`), usado como referência de "padrão que já funciona" pra corrigir o do Superadmin, na verdade tem a mesma lacuna — só limpa o cookie no client, nunca chama `/api/auth/logout` — registrado aqui pra não se perder | Mesma classe de risco já documentada nas auditorias anteriores (Lote 17, pós-Lote 17, fluxo completo, correção pós-auditoria): um botão de UI ligado a `alert()`/placeholder, ou a um dado no formato errado, não quebra a compilação nem aparece numa revisão de código isolada por arquivo — só aparece testando a ação real, ponta a ponta, pelo navegador |

...
async function handleRecuperacaoSenha(request: Request, env: Env): Promise<Response> {
  try {
    ...
    const corretor = await env.DB.prepare(
      "SELECT id, email FROM corretores WHERE email = ?"
    ).bind(dados.email.toLowerCase()).first();
    ...
    // Nota: em produção, armazenar em D1 com timestamp de expiração
    ...
    // TODO: Armazenar token em tabela de redefinições de senha (será criada no Lote seguinte)
    // TODO: Enviar e-mail via Resend com link: /api/auth/redefinir-senha?token={tokenRedefinicao}

    console.log(`Token de redefinição para ${corretor.email}: ${tokenRedefinicao} (expira em ${expiracao.toISOString()})`);
